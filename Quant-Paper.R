...
model_results <- quantmodel(set1)   #Change input set as and when required
set7<-set7[c(7:13)]
...
plot_country<-ggplot(country_summary7, aes(x=`Political Voilence is justifiable`, y=`Death Penalty is justifiable`, label = Cntry_AN))+ 
  geom_point(aes())+
  geom_smooth(method=lm)+
  geom_text(vjust = -0.5)+
  xlab("Political Voilence is justifiable") +
  ylab("Death Penalty is justifiable") +
  ggtitle("Relation between jusification of Political Violence and Death Penalty")

plot_country

#Coplot with 1% data
set_sample<-sample_n(country_summary1,1300)
coplot_country<-coplot(`State of Health`~ `Feeling of Happiness`|Cntry_AN, type="b", show.given = FALSE,data = set_sample)
